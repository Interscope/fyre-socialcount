...
define('iga/apps/fyre-socialcount/requestQueue',["jquery", "underscore", "base64", "iga/apps/fyre-socialcount/models/CurationCount", "iga/apps/fyre-socialcount/models/ContentCount"],
	...
	requestQueue.prototype.get = function(callback){
		...
		var requestParams = {}, model, batchSize = 1 || options.batchSize, batchRequestLimit = 0, batchRequestTimeout = 1;
		...
		_.each(_batches, function(batch, i){
			...
			if(options.shim){
				...
				if(batchRequestLimit && batchRequestLimit % (i+1) === 0 ){//ContentCount API only allows 10 requests per sec
					setTimeout(_request, batchRequestTimeout*batchRequestLimit/(i+1));
				}else{
					_request();
				}
			}
...
define('css!iga/apps/fyre-socialcount/css/fyre-socialcount.min.css', ['css-embed'], 
function(embed)
{
	embed(
	'div.iga-socialcount-container .bar{color:#fff;clear:both}div.iga-socialcount-container .bar:hover{opacity:.8}div.iga-socialcount-container .bar .width-percentOfTotal{background-color:#2558FF;display:block;position:relative;margin:1px 0 0;padding:.5em 4em .5em .5em;-webkit-transition:background-color .3s ease-out;-moz-transition:background-color .3s ease-out;transition:background-color .3s ease-out}div.iga-socialcount-container .bar .width-percentOfTotal a,div.iga-socialcount-container .bar .width-percentOfTotal a:visited{color:#fff;-webkit-transition:color .5s;-moz-transition:color .5s;transition:color .5s}div.iga-socialcount-container .bar .width-percentOfTotal a:hover{color:#3f3f44}div.iga-socialcount-container .bar .percentOfTotal{position:absolute;top:25%;right:0;padding-right:5px}'
	);
	return true;
});
...
define('iga/apps/fyre-socialcount/SocialCountCampaignView',["jquery", "underscore", "IGA.utils", "hogan", "backbone",
        ...
        "css!iga/apps/fyre-socialcount/css/fyre-socialcount.min.css",],
		...
		initialize: function(options){
			...
			function _$setAttr($attr, value, attrPath){
				...
				if($attr.is("meta")){
					...
					$attr.text(value);
					$attr.addClass("changed");
					setTimeout(function(){$attr.removeClass("changed");}, options.changeAnimationTimeout || 2000);
				}
			}
